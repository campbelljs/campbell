...
exports.lifecycleMixin = function (Campbell) {
  ...
  Campbell.prototype.$destroy = async function () {
    if (this._lifecycle.state.isBeingDestroyed) {
      // FIXME throw warning ?
      return;
    }
    await this._lifecycle.callHook("beforeDestroy");
    // begin destroy procedure
    this._lifecycle.state.isBeingDestroyed = true;
    // destroy children
    // TODO: destroy in parallel or sequentially ?
    await Promise.all(
